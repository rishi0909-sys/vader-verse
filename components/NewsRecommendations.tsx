...
function NewsCard({ rec, setExplainItemId }: { rec: RecommendedItem, setExplainItemId: (id: string) => void }) {
  ...
  const imageUrl = rec.item.image || rec.item.urlToImage || `https://picsum.photos/seed/${slug}-news/400/200`;
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0">
        <div 
          ref={imageRef}
          className="absolute inset-0 bg-cover bg-center pointer-events-none transition-transform duration-700 group-hover:scale-105"
          style={{ 
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/80 to-black/20 pointer-events-none group-hover:via-black/60 transition-colors duration-500" />
      </div>
      
      {/* Content Overlay */}
      <div className="p-5 flex flex-col justify-end h-full min-h-[220px] relative z-10 pointer-events-none">
        <h3 className="font-bold text-sm mb-3 line-clamp-2 text-white/90 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] transition-colors group-hover:text-white" title={rec.item.title}>{rec.item.title}</h3>
        <p className="text-xs text-white/60 mb-4 line-clamp-2 flex-1 font-medium drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]">{rec.item.content || rec.item.description}</p>
        ...
        <div className="mt-auto pt-4 border-t border-white/10 flex justify-between items-center relative z-20 pointer-events-auto">
          <button 
            ...
            className="flex items-center gap-1.5 text-xs font-semibold text-yellow-500 hover:text-yellow-400 transition-colors bg-black/40 px-2 py-1 rounded border border-yellow-500/20"
          >
            <HelpCircle className="w-3.5 h-3.5" /> Why this?
          </button>
          {rec.item.url && (
            <a href={rec.item.url} target="_blank" rel="noreferrer" className="text-xs font-bold text-white/60 hover:text-white transition-colors bg-white/5 px-2 py-1 rounded border border-white/10 backdrop-blur-md">
