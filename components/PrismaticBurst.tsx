...
float layeredNoise(vec2 fragPx){
    ...
    if (uQuality > 0.3) n += 0.20 * hash21(q * 4.0 + 47.0);
    if (uQuality > 0.6) n += 0.10 * hash21(q * 8.0 + 113.0);
    if (uQuality > 0.8) n += 0.05 * hash21(q * 16.0 + 191.0);
    return n;
}
...
void main(){
    ...
    int maxIters = int(44.0 * max(0.3, uQuality));
    for (int i = 0; i < 44; ++i) {
        if (i >= maxIters) break;
        vec3 P = marchT * dir;
        P.z -= 2.0;
...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    program.uniforms.uQuality.value = activeShaderQuality;
